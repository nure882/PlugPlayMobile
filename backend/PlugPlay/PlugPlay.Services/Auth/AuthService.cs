...
public class AuthService : IAuthService
{
    ...
    public async Task<Result<User>> ValidateUserCredentials(string email, string password)
    {
        ...
        {
            return Result.Fail<User>($"Error: {e.Message}");
        }
    }
    ...
    public async Task<Result<User>> GetOrCreateUser(
        string payloadEmail,
        string payloadGivenName,
        string payloadFamilyName,
        string payloadSubject)
    {
        ...
        try
        {
            user = await _userManager.FindByLoginAsync("Google", payloadSubject);
            if (user != null)
            {
            ...
            var userCreateResult =
                await CreateNewUser(payloadEmail, payloadGivenName, payloadFamilyName, payloadSubject);
    ...
    public async Task<Result<RefreshTokenResponse>> RefreshTokenAsync(string token)
    {
        ...
        {
            return Result.Fail<RefreshTokenResponse>($"Error: {e.Message}");
        }
    }
        ...
        string payloadSubject)
    {
        ...
        var newUser1 = new User
        {
            ...
            FirstName = payloadGivenName,
            LastName = payloadFamilyName,
            PhoneNumber = "",
            EmailConfirmed = true,
            CreatedAt = DateTime.UtcNow,
            Role = Role.User
